...
public class Gestore {
	 ...
	 * immettendo le sue generalit� tramite un opportuno menu che si 
	 ...
	 * ovvero l'et�, lo username e la password. Quando queste tre 
	 * propriet� dell'utente rispettano i vincoli la funzione inserisce
	...
	public void iscrivi(UtenteRegistrato u) throws IOException {
		boolean usn = false, psw = false, eta = false;
		if(checkEta(u.getEta()) == true) {
			eta=true;
		} else {
			eta=false;
		} 
		...
		if(eta==false)	{
			System.out.println("Et� non corretta");
		}
		if(usn==false)	{
			System.out.println("Username errato o utente gi� esistente");
		}
		if(psw==false)	{
			System.out.println("Password non corretta");
		}
		if((eta==true) && (usn==true) && (psw==true))	{
	 ...
	 * che verrano poi verificate e se corrette il login andr� a buon fine
	 * @throws IOException _
	 */
	...
	/**
	 * Funzione che presi i dati del libro in esame cerca se � presente
	 * nel catalogo della biblioteca
	 * @param l Dati libro da cercare
	 * @throws IOException
	 */
	public void ricerca(Libro l) throws IOException{
		boolean noISBN = false, noTit = false, noAut = false, noCed = false;
		if ( l.getISBN().equals("") )	{
			noISBN = true;
		}
					...
					if(libriRicerca.size()>1){
						...
						while(error==true){
							...
							if(c>=1 && c<=libriRicerca.size()){
								...
								int v=0;
								try {  
									v = Integer.parseUnsignedInt(stream);
								} catch (NumberFormatException ec) {
									System.out.println("Stringa errata inserire un valore numerico e positivo per indicare la scelta"); 
								...
								if(Login==true)	{
									if ( recensionelogin(0,v,stream,rec,utentelogin) ) {
										System.out.println("Recensione effettuata con successo");
										break;
									} else {
										System.out.println("Recensione fallita");
									}
								}
								else {
									if ( recensione(0,v,stream,rec) ){
										System.out.println("Recensione effettuata con successo");
										break;
									} else {
										System.out.println("Recensione fallita");
					...
					if(libriRicerca.size()>1){
								...
								for(int i=0 ; i<catalogo.getLibri().size() ; i++) {
									if ( libriRicerca.get(c-1)==catalogo.getLibri().get(i) ) {
										...
										if(rec.isDisponibile()){
											System.out.println();
											System.out.println("Il libro scelto e' disponibile alla consultazione"); 
											System.out.println();
						...
						for(int i=0 ; i<catalogo.getLibri().size() ; i++) {
							if ( libriRicerca.get(0)==catalogo.getLibri().get(i) ) {
								...
								if(rec.isDisponibile()){
									System.out.println();
									System.out.println("Il libro scelto � disponibile alla consultazione"); 
									System.out.println();
									boolean abbonato = false;
									for( int j=0 ; j<abbonamenti.size() ; j++){
										if(abbonamenti.get(j).getPagato()){
	 ...
	 * @throws IOException _
	 */
	public boolean recensione(int n, int v, String d, Libro l) throws IOException{
	 ...
	 * @throws IOException _
	 */
	public boolean recensionelogin(int n, int v, String d, Libro l, UtenteRegistrato u) throws IOException{
		Recensione r = new Recensione();
		r.setVoto(v); 
		r.setDescrizione(d);
		r.setLibro(l);
	...
	public boolean checkUsername(String u){
		if(u.length()<4){	//nome utente non pu� essere piu piccolo di 4 caratteri
			return false;
		}
	...
	/**
	 * Metodo che controlla in fase di iscizione se l'utente ha un'et� 
	 * minima di 13 anni
	 * @param e Valore et�
	 * @return Esito sulla correttezza Et�
	 */
	public boolean checkEta(String e)  {
		int eta=0;
		try {  
			eta = Integer.parseUnsignedInt(e);
		} catch (NumberFormatException ec) {
			System.out.println("Stringa errata inserire un valore numerico positivo per indicare l'eta"); 
			return false;
		}
		if(eta<13){
			return false;
	...
	public void menuSuggerisciAcquisto() throws IOException{
		...
		if(noISBN==true && noTit==true && noAut==true && noCed==true && noAed==true){ //non sono stati inseriti i campi
			System.out.println();
			System.out.println("Tutti i campi sono vuoti non � possibile inserire l'elemento nella richiesta");
			System.out.println();
		}
		else{
			System.out.println();
	...
	/**
	 * Metodo che controlla se c'� un utente collegato al momento
	...
	/**
	 * Permette di impostare la variabile booleana che verifica se un'utente � registrato
